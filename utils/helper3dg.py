...
def getparser():
    ...
    if os.path.exists(args.configpath) and args.configpath != "None":
        ...
        for k in config.keys():
            # Prioritize parameter explicitly given with commandline argument
            if "--" + k in sys.argv:
                print("skip config: " + k)
                continue
            try:
            ...
            if k == "test_iteration":
                if config[k] not in args.save_iterations:
                    args.save_iterations.append(config[k])
                    args.save_iterations.sort()
        print("finish load config from " + args.configpath)
    else:
        raise ValueError("config file {} not exist or not provided".format(args.configpath))
...
def getrenderparts(render_pkg, get_raw=False):
    render = render_pkg["render"] if not get_raw else render_pkg["raw_render"]
    return render, render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
...
def gettestparse():
    ...
    parser.add_argument("--rgbfunction", type=str, default = "rgbv1")
    parser.add_argument("--trbfp_inverse", action="store_true")
    ...
    parser.add_argument("--max_timespan", type=float, default = 1000.0)
    parser.add_argument("--min_timespan", type=float, default = -1000.0)
